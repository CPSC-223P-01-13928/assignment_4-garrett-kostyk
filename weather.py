...
def report_historical(data):
    ...
    for key in data:
        if x == key[0:8]:
            ...
            x = key[0:8]
            m = calendar.month_name[int(x[4:6])] + " " + str(int(x[6:8]))+ ", " + str(int(x[0:4]))
            min_temp = min_temperature(data = data, date = x)
            max_temp = max_temperature(data = data, date = x)
            min_hum = min_humidity(data = data, date = x)
            max_hum= max_humidity(data = data, date = x)
            rain = tot_rain(data = data, date = x)
